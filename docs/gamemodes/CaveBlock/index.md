...
=== "world.nether.roof"
    !!! summary "说明"
        允许切换下界顶部方块是否应为基岩方块。否则，它将由下界岩制成。

=== "world.nether.floor"
    !!! summary "说明"
        允许切换下界底部方块是否应为基岩方块。否则，它将由下界岩制成。

=== "world.nether.main-block"
    !!! summary "说明"
        允许设置将用于下界世界生成的主要方块。将其设置为空气将创建虚空世界。矿石脉（古代碎片、下界石英、黑曜石、荧石等）由脉填充器在其顶部放置。
...
!!! tip
    `[player_command]` 和 `[admin_command]` 是根据您运行的游戏模式而有所不同的指令。
    
    游戏模式的 `config.yml` 文件包含允许您修改这些值的选项。
    
    例如，在 CaveBlock 中，默认的 `[player_command]` 是 `cave`，默认的 `[admin_command]` 是 `cba`。
    
    请注意，此插件允许在插件的 `config.yml` 文件中更改玩家指令别名。


默认情况下，BentoBox 游戏模式插件带有默认的子指令集，但是，每个插件可能会引入更多子指令。

[完整的 CaveBlock 指令列表](Commands)
...
??? note "v1.23.1 新内容"
    **发布于：** 2026-07-09
    ...
    一个修复版本，堵住了 1.23.0 中引入的结构抑制漏洞。建议所有运行 1.23.0 且禁用了原版结构的服务器更新。

    - 🔺 **被禁用的结构不再冻结服务器。** 禁用结构此前只取消了结构的*放置*，而没有取消其放置*规则*，因此结构搜索（`/locate`、末影之眼、探险家/藏宝图、海豚、村民地图交易）仍会一直扫描到半径上限并冻结主线程。新增的 `StructuresLocateEvent` 处理器现在会在搜索前先移除被禁用的结构，立即返回"未找到"。修复 [#116](https://github.com/BentoBoxWorld/CaveBlock/issues/116)。
    - 🔺 **结构不再在出生点区域漏网。** 抑制监听器现在会在 `createWorlds()` 中提前注册，早于首批出生区块的生成，因此被禁用的结构不会再出现在出生点附近。

    [发布 v1.23.1](https://github.com/BentoBoxWorld/CaveBlock/releases/tag/1.23.1)
