...
这是正常行为，不是 bug。从 **BentoBox 3.16.1** 起，`/[admin_command] delete <player>`（以及 `/island reset`）会立即将岛屿从数据库中移除——玩家可以直接创建新岛屿——但**方块的清理会在下一次房屋管理扫描时进行**（默认每 24 小时）而不是立即进行。此期间该区域会被标记为"已删除"，你会看到*等待区域清理*的消息。

清理延迟是因为 BentoBox 删除了底层的区域文件，而一个区域文件可以容纳**多个紧密排列的岛屿**。BentoBox 只有在该区域文件中**没有任何活跃岛屿**后才会删除该文件，所以如果已删除岛屿的区域中仍然有活跃邻居岛屿，该已删除岛屿会一直保留在地图上，直到这些邻居岛屿也被清理。

要更快地清理：

- **立即强制执行扫描：**运行 `/bbox admin purge deleted`。它会清理每个区域文件现已为空的软删除岛屿——与活跃岛屿共享区域的岛屿会被跳过。
- **共享区域，需要立即移除：**使用 **WorldEdit**（`//pos1`/`//pos2`/`//set air`）手动移除方块或重新生成区块。岛屿已从数据库中删除；这仅是清除方块的外观操作。
- **清理后重启服务器**，这样 Paper 的区块缓存会被清空，空白区域会停止渲染。

有关完整的生命周期，请参阅[岛屿删除（管理员）](BentoBox/About/IslandManagement.md#island-deletion-admin)。
...
玩家必须先通过 `/[player_command] generator` **激活**一个生成器，并从 GUI 中选择一个。仅仅放置岩浆和水而不激活生成器只会产生原版鹅卵石。
