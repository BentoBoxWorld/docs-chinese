...
两大占用空间的因素是：(1) 从未回来的玩家生成的区块；(2) 在 `keep-previous-island-on-reset: true` 时重置后遗留的旧岛屿区域。回收空间的方法：

- 在 BentoBox 配置中设置 `keep-previous-island-on-reset: false`，使重置的岛屿被删除而不是废弃。
- 使用 `/[admin_command] purge` 删除不活跃玩家的岛屿，然后运行 `/[admin_command] purge regions` 删除实际的世界区域文件。**操作前务必备份世界文件夹。**
