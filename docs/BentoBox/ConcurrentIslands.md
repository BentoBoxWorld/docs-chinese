...
以这种方式指定岛屿意味着管理员不再需要站在岛上，因此该命令可以从**服务器控制台**运行——从而也可以通过诸如 Skript 之类的自动化运行。从控制台运行时，确认提示会被跳过，因为控制台无法作出应答。

- `<newOwner>` — 将成为所有者的玩家。他们**不**需要在线，也不需要站在任何地方；任何 BentoBox 已知的玩家（或原始 UUID）均可。
- `<islandOwner>` — 当前的所有者，仅用于确定要转移**哪一个**岛屿。该命令会转移这名玩家实际拥有的岛屿；如果其活动岛屿是属于他人的团队岛屿，则转移会被拒绝。如果他们拥有多个岛屿，则使用其**主要／活动**岛屿。
