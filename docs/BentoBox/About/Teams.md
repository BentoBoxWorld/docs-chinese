...
自 BentoBox 3.21.0 起，收到团队邀请时可以自动弹出一个模态的**[接受] / [拒绝]**对话框，玩家不必留意聊天消息再去输入接受或拒绝命令。该功能需要 Minecraft 26+ 的服务端，由 BentoBox `config.yml` 中的 `island.dialogs.team-invites` 控制（默认 `true`）。在更旧的服务端上，或该开关关闭时，邀请仍以常规聊天消息的形式送达。

敏感命令的确认——例如 `team kick` 和 `team leave`——同样可以显示为**[确认] / [取消]**对话框，而不必让玩家再输入一遍命令。这由 `island.dialogs.confirmations` 控制（默认 `true`）；只要对话框不可用，就会回退到"再输入一遍"的行为。
