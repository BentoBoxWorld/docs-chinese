...
蓝图通过**蓝图管理器**管理，并可以被打包成一个**蓝图包**，包含多达3个岛屿（普通世界、下界和末地）。蓝图包可以有自己的图标、描述，并可以包含其他设置，比如使用权限的要求。
...
5. 使用管理员蓝图指令pos1和pos2选择岛屿（记住使用蓝图指令，而不是WE！）
6. 使用管理员蓝图复制指令复制岛屿
7. 使用管理员蓝图保存指令保存岛屿
8. 重复上述步骤，包括下界和末地岛屿。
...
## 指令
指令几乎与WorldEdit的图纸指令相同。你必须是Op或拥有权限的管理员才能使用蓝图。使用管理员指令和**blueprint**或其别名**bp**：
...
* /bsb bp save <name> - 将剪贴板保存到文件中（保存为`.blueprint`纯文本 JSON 文件）
* /bsb bp load <name> - 加载一个蓝图文件（不要添加`.blueprint`或`.blu`后缀）
* /bsb bp - 打开蓝图管理器GUI

对于AcidIsland，请使用/acid而不是/bsb。

## 蓝图管理器GUI
蓝图管理器GUI使你能够创建、编辑和配置玩家在开始新岛屿或重置时可以选择的岛屿集。这些岛屿集（普通世界、下界世界和末地世界）被称为"包"。有一个默认的包可以自定义，但不能删除。
...
## 文件和编辑
在游戏中使用蓝图时，总是只使用蓝图的名称。在文件系统上，蓝图现在以**纯文本 JSON**格式保存，后缀为`.blueprint`。蓝图包同样保存为`.json`文本文件。`.blueprint`文件和包的`.json`文件都可以用任何文本编辑器编辑。
...
不必拥有下界或末地世界蓝图，但如果没有，那些世界将不会粘贴任何岛屿（显然）。
...
### 出生地标志
在你希望玩家出生的位置放置一个第一行为[spawn_here]的标志。他们将出现在这个位置，标志将不被粘贴。这适用于所有世界岛屿，所以你可以指定玩家在通过门进入下界时会出现的位置，例如。
